...
class GNNModule(HeterogeneousAnswering):
    def __init__(self, config, iteration=None):
        ...
        self.model_dir = os.path.join(self.data_path, self.benchmark, self.faith_or_unfaith, self.config["gnn_model_dir"])
        Path(self.model_dir).mkdir(parents=True, exist_ok=True)
        self.gnn_model_path = os.path.join(self.model_dir, self.config["gnn_model_file"])
    ...
    def train(self, sources=("kb", "text", "table", "info"), train_path=None, dev_path=None):
        ...
        # data path for training the GNN models
        ...
        if not train_path:
            ...
            train_path = os.path.join(
                data_dir, benchmark, tqu, fer, sources_str, train_file)
        ...
        if not dev_path:
            ...
            dev_path = os.path.join(
                data_dir, benchmark, tqu, fer, sources_str, dev_file)
        ...
        for epoch in range(epochs):
            ...
            if avg_dev_qa > min_dev_qa:
                ...
                Path(self.model_dir).mkdir(parents=True, exist_ok=True)
                torch.save(self.gnn.state_dict(), self.gnn_model_path)

                # saving the model (for not over-writing results)
                model_path = os.path.join(self.model_dir, f"model-{START_DATE}-{random_integer}.bin")
                torch.save(self.gnn.state_dict(), model_path)

                # saving the config
                config_path = os.path.join(self.model_dir, f"config-{START_DATE}-{random_integer}.json")
                with open(config_path, "w") as fp:
                    fp.write(json.dumps(self.config, indent=4))

                # saving the current result
                result_path = os.path.join(self.model_dir, f"result-{START_DATE}-{random_integer}.res")
