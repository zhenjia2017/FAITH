...
then
	...
	echo "Usage: bash scripts/seq2seq_iques.sh\\
		--train\\
		[<PATH_TO_CONFIG>]"
	exit 0
fi
...
if [[ $# -lt 3 ]]
then
	OUT="out/${DATA}/seq2seq-iques-${FUNCTION_NAME}-${CFG_NAME}.out"
fi
...
then
	...
	export FUNCTION CONFIG SOURCES OUT
  	nohup sh -c 'python -u faith/temporal_qu/implicit_resolver/seq2seq_iques/seq2seq_iques_module.py ${FUNCTION} ${CONFIG}' > $OUT 2>&1 &
else
	# run with sbatch
	sbatch <<EOT
#!/bin/bash
...
python -u faith/temporal_qu/implicit_resolver/seq2seq_iques/seq2seq_iques_module.py $FUNCTION $CONFIG
EOT
fi
